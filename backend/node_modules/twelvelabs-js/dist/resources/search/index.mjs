...
import { attachFormFile, convertKeysToSnakeCase, removeUndefinedValues } from "../../util.mjs";
export class Search extends APIResource {
    async query({ indexId, query, queryText, queryMediaType, queryMediaFile, queryMediaUrl, options: searchOptions, groupBy, threshold, operator, conversationOption, filter, pageLimit, sortOption, adjustConfidenceLevel, }, options = {}) {
        if (!queryText && !queryMediaFile && !queryMediaUrl) {
            if (query) {
                // deprecated; call /search endpoint
                console.warn('Warning: `query` is deprecated. Use `queryText`, `queryMediaFile` or `queryMediaUrl` instead.');
                ...
                const res = await this._post('search', removeUndefinedValues(_body), options);
        ...
        if (conversationOption)
            formData.append('conversation_option', conversationOption);
        if (filter)
        ...
        if (adjustConfidenceLevel)
            formData.append('adjust_confidence_level', adjustConfidenceLevel);
        try {
            if (queryMediaFile)
